 ...
 *
 * Colonne droite : contacter, contacts détaillés, statistiques,
 * abonnés, vérifications Shopi, correspondants similaires.
 * ================================================================ */
...
/* Couleur d'avatar pour les abonnés (mini) */
const FOL_COLORS = ['#3B0764', '#1549B8', '#047857', '#B45309', '#7C3AED'];
      ...
      <div className={styles.card}>
        ...
        <div className={styles.cb}>
          <div className={styles.folWrap}>
            {['MK', 'IB', 'FK', 'AS', 'MC'].map((ini, i) => (
              <div key={ini} className={styles.folAva} style={{ background: FOL_COLORS[i] }}>{ini}</div>
            ))}
            <div className={styles.folAva} style={{ background: '#5A7A9E' }}>+1K</div>
            <span className={styles.folCnt}>{abonnes.toLocaleString('fr-FR')}</span>
          </div>
          <button
            className={`${styles.sideBtn} ${suivi ? styles.sbCall : styles.sbPrimary}`}
            onClick={onToggle}
          >
            {suivi ? <><i className="fas fa-user-check" /> Abonné</> : <><i className="fas fa-plus" /> Suivre ce correspondant</>}
          </button>
        </div>
      </div>
          ...
          {similaires.map(s => (
            <div key={s.id} className={styles.simItem} onClick={() => navigate(`/correspondants/${s.id}`)}>
              ...
              <div className={styles.simRight}>
                ...
                <button className={styles.simFbtn} onClick={e => { e.stopPropagation(); onToast(`✅ Abonné à ${s.nom}`); }}>
                  + Suivre
                </button>
              </div>
            </div>
